...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        function showError(input, message) {
            const existingError = input.parentElement.querySelector('.' + errorClass);
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            contactForm.querySelectorAll('.' + errorClass).forEach(el => el.remove());
            ...
            contactForm.querySelectorAll('[required]').forEach(input => {
                ...
                if (input.name && (input.name.toLowerCase().includes('name') || input.name.toLowerCase().includes('الاسم')) && value && value.length < 2) {
                    showError(input, 'يجب أن يحتوي الاسم على حرفين على الأقل');
                    isValid = false;
                }

                // Message/textarea validation (minimum 10 characters)
                if (input.tagName === 'TEXTAREA' && value && value.length < 10) {
                    showError(input, 'يجب أن تحتوي الرسالة على 10 أحرف على الأقل');
                    isValid = false;
                }
            });
            ...
            if (isValid) {
                ...
                if (submitButton) {
                    ...
                    setTimeout(function() {
                        ...
                        successMessage.className = successClass;
                        ...
                        const existingSuccess = contactForm.querySelector('.' + successClass);
                        ...
                        setTimeout(function() {
                            const msg = contactForm.querySelector('.' + successClass);
                            if (msg) msg.remove();
                        }, 5000);
                    }, 1500);
                }
        ...
        const bodyObserver = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType === 1) {
                        if (node.matches && (node.matches('.fade-in, .animate-on-scroll, [data-animate]'))) {

                            node.style.transform = 'translateY(30px)';
                            node.style.transition = 'opacity 0.6s ease-out, transform 0.6s ease-out';
                            
                            const dynObserver = new IntersectionObserver(function(entries) {
                                entries.forEach(function(entry) {
                                    if (entry.isIntersecting) {
                                        entry.target.classList.add('visible', 'animated');
                                        entry.target.style.opacity = '1';
                                        entry.target.style.transform = 'translateY(0)';
                                        dynObserver.unobserve(entry.target);
                                    }
                                });
                            }, observerOptions);
                            dynObserver.observe(node);
                        }
                        
                        if (node.querySelectorAll) {
                            const children = node.querySelectorAll('.fade-in, .animate-on-scroll, [data-animate]');
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b3f1129e-dbab-4f78-b903-a5de6278b7f2',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b3f1129e-dbab-4f78-b903-a5de6278b7f2';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b3f1129e-dbab-4f78-b903-a5de6278b7f2';
